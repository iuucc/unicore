...
> | 12 | 19173.6 | 3208.36 |
>
> **原结论的口径有错，但方向是对的——下面是订正后的完整推理。**
>
> 本节原先写："单通道情形更糟（GPU 477 samples/s vs 加载 310 samples/s，GPU 约 35% 时间闲置）"。
> 这个判断**方向正确，但两个数字口径不一致**：310 是 **C=34** 的加载速率，477 是 **C=1** 的 GPU 速率。
> 实测 C=1 的加载速率是 **924.95 samples/s**（`num_workers=0`）。
>
> **但不能据此推翻原结论。** 训练循环里有**每步的 CPU↔GPU 同步**
> （`first_experiment.py` 的 `if not torch.isfinite(grad_norm)`），同步点使加载与计算**无法重叠**，
> 于是每 batch 的实际开销 = 加载 + 计算，**与"两者谁更快"无关**。
> 因此判据必须是**训练墙钟**，不能拿"加载基准 vs GPU 基准"来论证。
>
> 空载机器上的受控 A/B（`--epochs 2 --train-samples 65536 --eval-samples 500 --batch-size 64`，
> 两个 epoch 同属 joint 阶段，除 `num_workers` 外参数逐字一致；日志 `runs/_env/ab2_numworkers.log`）：
>
> | | `num_workers=0` | `num_workers=8` |
> |---|---:|---:|
> | epoch 1（含一次性启动与 cudnn 自动选算法） | 271 s | 273 s |
> | **epoch 2（稳态）** | **249 s** | **186 s** |
> | 评估阶段 | 36 s | 30 s |
>
> **稳态每 epoch 249 → 186 s：每 batch 节省 61.5 ms，下降 25.3%。**
> 而 C=1/bs=64 的加载耗时是 `64 / 924.95 ≈ 69 ms`，**两者吻合**，
> 说明省下的正是被同步点串行化掉的加载开销。
>
> 12 epoch 全量运行的逐阶段对照（同阶段比较，由 checkpoint 时间戳提取）也一致：
>
> | 阶段 | epochs | nw=0 | nw=8 | 每 batch 节省 |
> |---|---|---:|---:|---:|
> | decomposition | 2–6 | ~263 s | ~194 s | 67 ms |
> | residual | 7–9 | 200.8 s | 114.9 s | 84 ms |
> | joint | 10–12 | 300.5 s | 222.7 s | 76 ms |
>
> **结论：`num_workers=0` 在本机确实让 GPU 等数据**——单通道下每 batch 白等约 62–84 ms；
> `num_workers=8` 把它隐掉，稳态墙钟下降约 **25%**。
> 本节原有的方向性判断成立，需要更正的只是论证方式（不能用加载基准与 GPU 基准直接比大小）。
>
> 另：短测中 `num_workers=12 → 4412` 明显高于 `=4 → 2399`，长测里变成 `12 → 3208` 与 `4 → 2390`，
> 12 个 worker 的相对收益远没有短测显示的那么大；`8 → 12` 几乎无额外收益。
>
...
- DataLoader 稳态 samples/s：
  ...
  - → **注意：不要用"加载基准 vs GPU 基准"判断瓶颈**。训练循环有每步 CPU↔GPU 同步
    （`torch.isfinite(grad_norm)`），加载与计算不能重叠，**判据是端到端墙钟**。
    空载受控 A/B（2 epoch × 65536，同为 joint 阶段）：稳态每 epoch
    **249 s（nw=0）→ 186 s（nw=8）**，每 batch 省 61.5 ms（↓25.3%），
    与 C=1/bs=64 的加载耗时 `64/924.95 ≈ 69 ms` 吻合——**单通道同样吃满这个收益**。
...
| 2026-09-22 | T0.7 / §2.2 | §2.2 字面要求"`scripts/` 下**所有**脚本必须支持 `--config`"，但纯工具脚本（`download_*` / `probe_pool` / `smoke_test` / `bench_hardware` / `run_dual_gpu`）没有实验参数，加空壳 flag 只是噪声 | 定为：**承载实验参数的入口**（`first_experiment.py`、`train.py`，以及阶段 1 起新增的 train/eval 脚本）必须实现 `--config` 与"命令行 > 配置 > 代码默认值"的优先级；纯工具脚本不加空壳 `--config`。该解释只放宽"所有"二字，不放宽 `--config` 机制本身 |
| 2026-09-22 | T0.7 / §5.3 | §5.3 要求"每个 run 目录必须含 `run_manifest.json`"，但 §8 的任务清单里**没有任何任务负责实现它** | 落在 T0.7：新增 `unicore_eeg/manifest.py`（`git_state` / `code_version_hash` / `write_run_manifest`），并在 `first_experiment.py`、`train.py` 于**训练开始前**写出 manifest 与 `config.yaml` 副本，使崩溃的运行也留有登记 |
...
| 2026-09-22 | §0.3.7 | 本节写"单通道情形更糟（GPU 477 samples/s vs 加载 310 samples/s，GPU 约 35% 时间闲置）"。**方向正确，但两个数字口径不一致**：310 是 **C=34** 的加载速率、477 是 **C=1** 的 GPU 速率 | §0.3.7 已整段更正。**结论仍是"`num_workers=0` 确实让 GPU 等数据"**，但论证换了正确的方式：训练循环里有每步的 CPU↔GPU 同步（`torch.isfinite(grad_norm)`），加载与计算无法重叠，故**判据是训练墙钟而非加载基准**。空载受控 A/B（2 epoch × 65536，同为 joint 阶段）：稳态每 epoch **249 s（nw=0）→ 186 s（nw=8）**，每 batch 省 61.5 ms（↓25.3%），与 C=1/bs=64 的加载耗时 69 ms 吻合。另修正短测 `num_workers=12 → 4412` 的乐观值（长测 3208） |
| 2026-09-22 | §0.3.7 | 执行 AI 一度把上一条"更正"成了相反的结论——**"C=1 下加载是 GPU 的 3.4 倍，不存在饿死"**，据此在报告中称 `num_workers` 对单通道"不带来吞吐收益" | **该相反结论是错的，已撤回。** 错因是只用"独立加载基准 vs GPU 基准"比较，忽略了训练循环的每步同步点。用 12 epoch 全量的逐阶段时长（decomposition/joint 每 batch 省 67–76 ms、residual 省 84 ms）与空载 A/B（省 61.5 ms）双重验证后订正。**教训：涉及"谁是瓶颈"的判断，必须用端到端墙钟，不能用分项基准比大小。** |
...
| 2026-09-22 | T1.1 | 手册与上一轮记录称 ds004784 的 128 个 EEG 通道名是 `A1`…`A128`。**实测不是**：`channels.tsv` 与 `electrodes.tsv` 都是 **`A1–A32` / `B1–B32` / `C1–C32` / `D1–D32`** 四扇区各 32 个（BioSemi ABCD 命名）；另有 8 个 `EXG` 与 128 个 `N-A*…N-D*`（MISC，无坐标） | 按实测命名写 `configs/montages/ds004784.yaml`；上一轮记录的错误已在本次更正。另：坐标**不是** MNE RAS，实测为 **x=前后（x<0 为枕/颈侧）、y=左右（严格镜像对称）、z=上（顶点 A1=(0,0,1)）**，且 128 个电极半径**恒为 56.0 mm**（标准差 2.7e-07）——是球面阵而非头皮帽 |
| 2026-09-22 | T1.1 步骤 4 | 决策 ⑧ 原定"`ds004784_19ch.yaml` 按三维坐标与标准 19 导做最近邻匹配"。**实测该规则的前提不成立**：体模的 128 电极是均匀铺满球冠的阵列，不是 10-20 头皮帽。用严格 ICP（60 次迭代 × 8 种初始朝向）求最优刚性对齐后，标准 19 导到最近电极的平均距离仍有 **0.1485 半径 ≈ 8.3 mm**；且阵列对 y **100% 镜像对称**，导致面内朝向不可解（y 翻转对结果零影响）；随机旋转对照显示该匹配几乎无判别力 | **经用户确认放弃该文件**（用户选择"放弃 19 导子集"）。跨数据集比对时 ds004784 按 128 导单列报告。理由与证据写入 `ds004784.yaml` 的 `notes`，并加回归测试 `test_ds004784_has_no_19ch_subset` 锁死该决定 |
| 2026-09-22 | T1.1 步骤 1 | 手册只说"实现 10-20 标准坐标表"，未说明数值来源 | 参考表由 `scripts/build_montages.py --emit-standard` 从 MNE **`standard_1020`（94 点）∪ `standard_1005`（仅补 1020 缺的 6 个名字 `FTT9h TTP7h TPP9h FTT10h TPP8h TPP10h`，openbmi 的 62 导用到）** 生成，落盘为 `configs/montages/standard_reference.yaml`。**归一化规则**：全部坐标除以标准 19 导的平均模长，使平均半径精确为 1（因 MNE 头模型不是正球，单点半径落在 0.847–1.257，保留了真实头形差异）。数值可复核：`test_reference_table_reproduces_mne` 逐点比对 MNE 原值（容差 1e-6） |
| 2026-09-22 | T1.1 步骤 2 | 手册要求 `resolve_montage(channel_names)`，但 bci2a 的 EDF 通道名是匿名的（只有 4 个带名，其余是 `EEG-0`…`EEG-16`），仅凭名字无法解析 | 新增 `resolve: positional` 模式：配置里给出记录的完整通道顺序 `raw_layout`（25 名）与 `anchors`（0→`EEG-Fz`、7→`EEG-C3`、9→`EEG-Cz`、11→`EEG-C4`、19→`EEG-Pz`）。**锚点校验不通过就抛 `MontageError`**，而不是默默按错位映射。已实测五个锚点位置与 `BCI2A_EEG_CHANNELS` 完全一致 |
| 2026-09-22 | T1.1 步骤 4 | 手册写"`bci2b.yaml`：3 双极（C3 Cz C4）"，但原始通道名为 `EEG:C3`/`EEG:Cz`/`EEG:C4`，没有第二个电极可供取中点 | 按标准位取坐标、`bipolar: false`（旧项目 `constants.py::BCI2B_EEG_CHANNELS` 同样把它们列为标准名）。已在配置的 `notes` 里写明与手册字面的差异 |
| 2026-09-22 | T1.1 步骤 4 | "其余数据集：T0.3 探测后补"——实测各库的通道构成比手册表格更细 | 除 4 个规定文件外，补出 8 个：`openbmi`(62 原始，缓存 20 是子集)、`physionet_mi`(64 原始，缓存 21 是前 21 个)、`cho_gigadb`(64，末两路 `FCz_ref`/`Oz_ref` 是参考通道)、`chbmit`(23 双极)、`cap_sleep`(18 路里**只有 13 路是 EEG 导联**，其余 ROC/EMG/ECG/DX/SX 列为 aux)、`sleep_edfx`(7 路里 2 路 EEG)、`ds002094`(30)、`faced`(32)。**分类按"能否解析出坐标"判定，而不是"名字里有没有 `-`"**（后者会把 `ROC-LOC`、`ECG1-ECG2` 误判成 EEG 双极） |
| 2026-09-22 | T1.1 步骤 5 | 双极导联取两端电极中点这条规则，在**跨半球**导联上会退化：`C4-A1`（右中央 vs 左乳突）的中点半径只有 **0.2104**，`FT9-FT10` 只有 **0.5412**，位置信息量极低 | 保留手册规定的中点规则，但 `coverage_report` 自动产出 `low_information` 字段（阈值 0.6）如实列出这两路，供下游决定是否降权；不把它们伪装成正常位置。回归测试 `test_low_information_positions_are_reported` 钉住 |
| 2026-09-22 | T1.1 | 旧项目 `cho.py::CHO_EEG_CHANNELS` 有 64 个名字，且数据集自带 `eeg.psenloc`（64×3，单位球）与 `eeg.senloc`（cm），看似可以拿到真实几何 | **实测拒绝使用自带坐标**：在已知通道对应下做 Kabsch 刚性对齐，最优平均弦长 **0.90**（完全随机约 1.27），说明 `psenloc` 的数组顺序与 `CHO_EEG_CHANNELS` 不对应、或不是头部坐标系。故 `cho_gigadb.yaml` 改用**按名解析**（64 个名字全部能解析，零近似）。理由写入配置 `notes` |
| 2026-09-22 | T1.1 / §6.5 | §6.5 的空间先验需要知道"前"与"上"是哪个轴，但手册未规定坐标系声明方式 | montage 配置新增 `frame_axes`（`up` / `anterior` 两个单位向量）。参考表为 MNE RAS `(0,0,1)`/`(0,1,0)`；ds004784 显式声明 `anterior=(1,0,0)`（否则前额优势先验会算错方向）。`Montage.frame_axes` 属性提供带默认值的读取，回归测试 `test_ds004784_frame_axes` 覆盖 |
| 2026-09-22 | T1.2 | 改造观测抽取器时把原实现的 `design.float()` / `y.mean(1).float()` 去掉了，训练在 epoch 1 第 0 个 batch 就崩：`NotImplementedError: "lu_factor_cublas" not implemented for 'BFloat16'` | 根因比"dtype 不对"更深：**`@`（matmul）是 autocast 的降精度算子**，即使把张量转成 float32，autocast 也会把它降回 bf16。修法是让抽取器**显式退出 autocast**（`model.no_autocast` 上下文），内部统一 float32、出入口再转回输入 dtype。附带发现 CPU 的 `fft.rfft` 也不支持 bf16（`RuntimeError: Unsupported dtype BFloat16`），所以"统一 float32"同时消除了跨设备差异。回归测试 `test_extractor_works_under_autocast` 用 CPU 的 bf16 autocast 复现该场景，**不必再等 50 分钟的训练崩在中途** |
| 2026-09-22 | T1.2 步骤 1 | 手册写"对每个通道独立 lstsq" | 实现改为**带 1e-6 岭项的正规方程**解 6×6 线性系统：比 `torch.linalg.lstsq` 快得多，且峰频退化到 0（基函数不满秩）时不会发散；岭项相对设计矩阵的 O(1) 量级可忽略。等价性由 `test_harmonic_fit_equals_direct_lstsq` 断言（容差 1e-3） |
| 2026-09-22 | T0.7 追加 | `scripts/run_dual_gpu.py` 的 GPU 采样线程把停止事件命名为 `self._stop`，**覆盖了 `threading.Thread._stop` 方法**，于是 `Thread.join()` 抛 `TypeError: 'Event' object is not callable`——任务失败时报告环节本身就崩，把真实错误盖掉了 | 事件改名为 `self._halt`，并在代码里写明这个陷阱。此后 T1.2 的回归才暴露出上面那条 bf16 错误 |
| 2026-09-22 | T1.5 步骤 2 | 手册要求 `regress_multichannel.py`"跑 T0.5 与 T1.2/T1.4 的对照" | 实现为：默认**只做对照**（三个 run 的指标并排 + 逐对绝对差 + PASS/FAIL 判定，输出 `runs/_regress/regress_multichannel.md`）；确实缺 run 时加 `--execute` 才会补跑。理由：一个 run 要 50 分钟，不该被一个"对照"脚本误触发。附表同时给出"相对 T0.5 基线"与"相对 `after_t07_numworkers8` 同口径"两组差，便于把 T1.2/T1.4 的效应与 T0.7 的 num_workers 效应分开 |
| 2026-09-22 | T1.3 | 手册 T1.3 验收写"`count_parameters` 在 C=2 与 C=128 下完全相同"，但**改造前的模型就不满足**：`ArtifactTokenizer` 的 `stat_dim = in_channels * 3` 会进入 `fuse` 的输入维度，再加上首/尾若干 `Conv1d(in_channels, …)`，实测 C=2→128 相差 **771,120** 个参数 | 按 **Gate 1** 的措辞（"`SpatialProjectionHead` 参数量与 C 无关"）判定：新模块参数量恒为 **1,444**，与 C 无关 ✅。整模型的 C 相关性用 `test_whole_model_parameter_growth_is_explained` 显式钉住（记录 771,120 这个数与它与 (C−2) 的正比关系），避免以后有新的 C 依赖层混进来而无人察觉 |
| 2026-09-22 | T1.3 / §6.4 | §6.4 步骤 2 算出了逐通道载荷 `(B, C, rank)` 之后，步骤 3–5 没有再用它；另 §6.4 要求"对 `y_norm` 做 SVD"，但 `C=128` 时每个 batch 要做 64 次 128×1000 的分解，代价过大 | 两处都做了处置并登记：**① 逐通道载荷真正进入 MLP 特征**（与坐标、RMS、有坐标标志拼成 8 维，仍与 C 无关），因为"空间投影一致性"的载体就是它；**② 默认改用确定性幂迭代**求同一子空间（`spatial_pca_mode="power"`，4 次迭代），精确 SVD 保留为 `"svd"` 供复核，`test_power_iteration_matches_exact_svd` 断言两条路径子空间一致（主角度余弦 > 0.98）。另新增 `spatial_coord_space`（`raw` 取前两分量 / `disk` 走等距方位投影），默认 `raw`——因为各数据集坐标系朝向不同，"前两个分量"在任何坐标系下都是稳定的二维编码 |
| 2026-09-22 | T1.3 | 空间权重直接调制观测会让"打开 `use_spatial`"在训练一开始就扰动既有行为，使单通道回归失去可比性 | 把 α 定义为 `spatial_modulation_alpha_scale · tanh(α_raw)`，**`α_raw` 初值 0** → 初始化时整个模块是恒等映射。`test_alpha_zero_is_identity` 断言"打开 use_spatial 的模型与关闭它的模型逐键输出一致"；`use_spatial=False` 时返回字典不多出空间相关键，使等价性回归可以直接比对整个输出字典。α 通过 tanh 有界，不会发散 |
| 2026-09-22 | T1.3 | 实现缺陷：正交化只做投影、**不逐列归一化**，导致参考向量范数指数放大（4 列就从 337 涨到 3.4e22），归一化时除以 inf 得到 NaN。C=1/2/3 时列数少看不出，C=34 才暴露 | 改为"投影完立刻归一化 + 第二遍精修"，并加 `test_orthonormalize_is_numerically_stable`（含 1e6 倍大范数输入）与 Gram 矩阵偏离单位阵 < 1e-3 的断言。另修掉 C < rank 时载荷维度不足会让 MLP 输入维度随 C 变化的问题（零列补齐） |
| 2026-09-22 | T1.2 | 未预期的性能现象：同 stage、同 `num_workers=8`、同规模下，逐 epoch 墙钟由 T0.7 对照的 ~178–199 s（decomposition 2–6）降到 **~94–101 s** | 两次运行唯一差异是 T1.2 的抽取器改造，最可能来源是替掉了 `torch.linalg.lstsq`。后续 T1.2 的正式回归全程墙钟 **1152 s**（12 epoch），对比 T0.7 的 **2330 s**，约 2× 加速属实。**归因仍未做受控验证**，只作观察记录，不作为任何验收依据 |
| 2026-09-22 | T1.4 步骤 2 | 手册写"样本字典新增 `mixing_matrix`：`(6, C)`"，但 §6.5 的伪码写 `A ∈ R^{C×K}` | 采用 **`(6, C)`**（行 = 伪迹族，与 `artifacts` 的第一维对齐），即 §6.5 伪码的转置；理由：与同一样本里 `artifacts[k, c, t]` 的索引顺序一致，下游做空间指标时不必转置。`SpatialMixer.build_matrix()` 内部仍按 §6.5 返回 `(C, K)`，样本里记录的是它的转置 |
| 2026-09-22 | T1.4 步骤 2 | 手册写"新增 `"coords": (C, 2)`" | 存 **`(C, 3)`**。二维投影依赖坐标系朝向（`project_to_disk` 假设 y=前），而 ds004784 的体模坐标系不是 RAS（x=前、y=左右）；把投影结果固化进数据集，等于把一个错误假设变成既成事实。模型自己按前两个分量取用（`SpatialProjectionHead.project_coordinates`），需要头地图时再显式投影 |
| 2026-09-22 | T1.4 步骤 5 | **缺陷**：手册要求"记录 A 为真值供空间指标评价"。但 `first_experiment` 模式下 condition 7–11 会把第 0–4 族分别留出并把能量重定向到下标 5；若照抄本次生成的 `A` 的第 5 列，记下的是 **unknown 源那一列**，而实际施加到下标 5 的是**另一个族**的权重——阶段 4 的空间指标会拿错误的 A 比对 | 两处处置：**①** 一个伪迹下标只允许收到一个源族的能量（held-out 族与 unknown 源互斥），语义也更干净——"下标 5 = 某一族未知伪迹"而非"两族之和"；**②** 记录改为 `applied[target_family] = matrix[:, family]`，未激活的族留全零行。判据：`test_mixing_matrix_records_what_was_applied`（某下标有能量 ⟺ 该行非零）＋ `test_held_out_family_prior_is_carried_into_the_unknown_row`（condition 8 留出 ocular 时下标 5 的行仍须是前额优势；condition 10 留出 cardiac 时仍须全通道同号） |
| 2026-09-22 | T1.4 | 手册 §6.5 给出的是逐族混音矩阵，但合成器需要**逐族**的多通道伪迹才能分别做 SNR 缩放（T1.4 步骤 4：SNR 作用在混音后的伪迹上） | `SpatialMixer` 增加 `mix_by_family()`（返回 `(K, C, T)` 与 `(C, K)`），`mix_sources()` 改为它的求和封装。合成器先逐族得到 `(C, T)`，再按各族目标 SNR 缩放后叠加 |
